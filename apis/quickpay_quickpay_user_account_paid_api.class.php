...
class quickpay_quickpay_user_account_paid_api extends Component_Event_Api {
	...
	private function user_account_paid($user_id, $order_id) {
		...
		/* 检查订单是否已经付款 */
		if ($order_info['pay_status'] == \Ecjia\App\Quickpay\Enums\QuickpayEnum::PAID && $order_info['pay_time']) {
		...
		/* 更新订单表支付后信息 */
		$data = array(
			'order_status'    => \Ecjia\App\Quickpay\Enums\QuickpayEnum::CONFIRMED,
			//'confirm_time'    => RC_Time::gmtime(),
			'pay_status'      => \Ecjia\App\Quickpay\Enums\QuickpayEnum::PAID,
