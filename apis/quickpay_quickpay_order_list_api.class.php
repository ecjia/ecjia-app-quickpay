...
class quickpay_quickpay_order_list_api extends Component_Event_Api {
	...
	public function call(&$options) {
		if (!is_array($options)) {
			return new ecjia_error('invalid_parameter', __('调用api文件，quickpay_order_list，参数无效', 'quickpay'));
		}
	...
	private function quickpay_order_list($options) {
		...
		$deleted_status = \Ecjia\App\Quickpay\Enums\QuickpayEnum::DELETED;
		$canceled_status = \Ecjia\App\Quickpay\Enums\QuickpayEnum::CANCELED;
		
		$db->where('order_type', 'quickpay');
		$db->where('order_status', '<>', $deleted_status);
		$db->where('order_status', '<>', $canceled_status);
